...
ARCS_FILE = "graph_data/arcs_oriented.csv"
...
def draw_arc(ax, p1, p2, radius, **kwargs):
    """Dessine un segment ou un arc circulaire à partir des points et du rayon avec flèche."""
    x1, y1 = p1
    x2, y2 = p2

    if math.isinf(radius) or radius == 0:
        # Segment droit avec flèche
        ax.annotate(
            "", xy=(x2, y2), xytext=(x1, y1),
            arrowprops=dict(arrowstyle="->", color=kwargs.get("color", "black"), lw=kwargs.get("linewidth", 1))
        )
        return
    ...
    # --- Ajouter une flèche pour indiquer le sens ---
    mid_idx = len(xs) // 2  # position de la flèche au milieu de l'arc
    ax.annotate(
        "", xy=(xs[mid_idx + 1], ys[mid_idx + 1]), xytext=(xs[mid_idx], ys[mid_idx]),
        arrowprops=dict(arrowstyle="->", color=kwargs.get("color", "black"), lw=kwargs.get("linewidth", 1))
    )
